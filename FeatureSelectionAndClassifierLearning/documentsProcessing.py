...
def tokenizeAndGetFreq4SingleDocument(pathToFile):
    resultDict = {}
    thisFile = open(pathToFile,'r') #open the file and process each file
    currentTextString = " ".join(thisFile.read().split())#store the file as a string for removing HTML tags
    textAfterHtmlRemovingString = re.sub('<[^>]*>', '', currentTextString) # remove HTML tags (String)    
    textAfterHtmlRemovingList = textAfterHtmlRemovingString.split() # convert String to List for the text contains only characters
    textRemoveingUnnecessaryCharactersList = [removeUnnecessaryCharacters(word) for word in textAfterHtmlRemovingList ] 
...
# Functions for sort and cut top most frequent words in tokenized test documents.
def sortAndCutWordsInTestSet(toBeProcessed,numberOfWordsToKeep):
    wordList= sorted(toBeProcessed.iteritems(), key=lambda frequency:frequency[1], reverse = True) # sort the dict based on the value (frequency)
    wordList = wordList[0:numberOfWordsToKeep] # the top 10 most frequent words
    return wordList